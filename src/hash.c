...
int main(int argc, char **argv) {
    ...
    if (argc < 3) {
        printf("Usage: %s <modulo> <string_1> [<string_2> ... <string_i>]\n", argv[0]);
        return 1;
    }
    
    table_size = atoi(argv[1]);

    printf("Creating FastHash Table (size: %d)\n", table_size);
    
    fht = fasthash_init(table_size, FH_NO_COLLISIONS | FH_NO_INDEXING, &test_destructor);
    
    
    for (i = 2; i < argc; i++) {
        index = fasthash_insert(fht, argv[i], (void *)i);
        if (index > fht->size) {
            printf("FastHash conflict occured!\n");
        } else {
            printf("table[\"%s\"] = table[%u] = (%d)\n", argv[i], index, i);
        }
    }
    
    for (i = 2; i < argc; i++) {
        fasthash_node *n = fasthash_lookup(fht, argv[i]);
        if (n) {printf("table[\"%s\"] = (%d)\n", argv[i], (int)n->data);}
    }
    
    printf("Cleaning up FastHash Table\n");
    
    fht = fasthash_cleanup(fht);
